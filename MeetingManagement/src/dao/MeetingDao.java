...
public class MeetingDao extends DataBaseFactory {
	...
	public int getCount(String meetingname, String roomname,
		...
		StringBuffer sb = new StringBuffer(
				"SELECT count(*) FROM meeting m,employee e,meetingroom mr WHERE m.roomid=mr.roomid AND m.reservationistid=e.employeeid");
	...
	public int insert(Meeting meeting) {
		...
		try {
			...
			ps = con
					.prepareStatement(
							"insert into meeting (meetingname,roomid,reservationistid,numberofparticipants,starttime,endtime,reservationtime,description,status) values (?,?,?,?,?,?,?,?,?);",
	...
	public Meeting getMeetingById(int mid) {
		...
		try {
			con = DBUtils.getConnection();
			ps = con
					.prepareStatement("select * from meeting WHERE meetingid=?;");
			ps.setInt(1, mid);
			rs = ps.executeQuery();
			if (rs.next()) {
				int meetingid = rs.getInt("meetingid");
				...
				String description = rs.getString("description");
				int status = rs.getInt("status");
				meeting = new Meeting(meetingid, meetingname1, roomid,
	...
	public int cancelMeeting(int mid, String reason) {
		...
		try {
			con = DBUtils.getConnection();
			ps = con
					.prepareStatement("UPDATE meeting  SET status=1,canceledtime=?,canceledreason=? WHERE meetingid=?");
